...
const CONDENSE_PROMPT =
`Given the conversation history below and a follow-up question, the task is to analyze the context provided by the chat history and the specific query by the user. Based on this analysis, rephrase the follow-up question into a standalone question that accurately reflects the user's current informational need. The standalone question should be formatted as "What is the timeline of xxx?".
